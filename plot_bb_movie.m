function h = plot_bb_movie(X, varargin)
...
if(nargin ==1)
	h(1) = plot(COM_x,COM_y,'--o','Color',[.4 .4 .4],'LineWidth',2,...
		'MarkerFaceColor','k','MarkerSize',15);
% 		'Color',[.7 .2 .18],'MarkerSize',15);
	hold on;
	h(2) = plot(body_x,body_y,'LineWidth',6,'Color',[.2 .2 .7]);
	h(3) = plot(ball_x,ball_y,'LineWidth',4,'Color',[.75 .4 .1]);
	hold off;
	axis equal;
else
	h = varargin{1};
